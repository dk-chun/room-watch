...
def transit(lat, lng):   # min(회사도보, 최근접셔틀 대중교통). 700m내/실패는 도보 폴백
    od = hav(lat, lng, OFFICE[0], OFFICE[1])
    best = (round(od / WALK_MPM), '회사도보', 'walk', round(od))
    nn, s = min(SHUTTLE.items(), key=lambda kv: hav(lat, lng, kv[1][0], kv[1][1]))
    dm = hav(lat, lng, s[0], s[1])
    if dm <= 700:
        cand = (round(dm / WALK_MPM), nn + '앞 도보', 'walk', round(dm))
    else:
        r = odsay_raw(lat, lng, s[0], s[1])
        cand = (r['min'], nn, 'transit', round(dm)) if r.get('min') else (round(dm / WALK_MPM), nn + '앞 도보', 'walk', round(dm))
    if cand[0] < best[0]: best = cand
    return {'tmin': best[0], 'tmode': best[1], 'ttype': best[2], 'tdist': best[3]}
...
def build_html(rows, report, ts):
 ...
 <div class="img" style="{imgstyle}"></div><div class="body">{badge}
  ...
  <div class="commute">🚆 약 {r.get('tmin')}분 · {esc(r.get('tmode'))}{f' <span class="dim">({r.get("tdist")}m)</span>' if r.get('ttype') == 'walk' else ''}</div>
