...
def brief(r):
    ...
    return {'id': r['id'], 'sales': r['sales'], 'deposit': r['deposit'], 'rent': r['rent'],
            ...
            'addr': r['addr'], 'approve': (r.get('approve') or '')[:4], 'commute': f"{cd}m {cv}" if cd else None,
            'link': link(r)}
...
def build_html(rows, report, ts):
    ...
    cards = []
    for r in sorted(rows, key=lambda x: x.get('cd') or 9999):
        st = r.get('_status', '')
 ...
 <div class="img" style="{imgstyle}"></div><div class="body">{badge}
  ...
  <div class="commute">🚉 {r.get('cd')}m {esc(r.get('cv'))}</div>
