      ...
      await Promise.all(${JSON.stringify(chunk)}.map(async no=>{
        try{ const r=await tf('/api/articles/'+no+'?complexNo=');
          ...
          out[no]={room:d.roomCount??'', bath:d.bathroomCount??'', move:(d.moveInTypeName||''), apt:(d.aptName||''), dbldg:(d.buildingName||''),
                   tc:(d.tradeCompleteYN||''), end:(d.exposeEndYMD||''), gone:(j.articleDetail?0:1)};
        }catch(e){ out[no]={gone:1}; }
      }));
      return JSON.stringify(out);
    })()`);
...
const rows = [...units.values()].map(ms=>{
  ...
  const stale = det.gone || det.tc==='Y' || (det.end && det.end < TODAY);   // 이미 내려간/거래완료/노출종료
  const path = a.realEstateTypeName==='오피스텔' ? 'offices' : 'houses';     // 종류별 정확한 딥링크
  return {
    no:a.articleNo, sales:a.tradeTypeName, type:a.realEstateTypeName, stale,
    ...
    link:`https://new.land.naver.com/${path}?articleNo=${a.articleNo}`
  };
}).filter(r=>!r.stale).sort((x,y)=>x.cd-y.cd);
...
console.error(`\n만료·거래완료 제외: ${units.size-rows.length}단위`);
console.error(`최종 ${rows.length}단위 · 전세 ${nJ} / 월세 ${nW} · 투룸+ ${rows.filter(r=>Number(r.room)>=2).length}`);
